...
- Only `/workspace` is mounted and persists across runs (tenant workspace). Everything else is ephemeral.
- Do NOT rely on background processes, cron, systemd, or in-memory state surviving after the run ends.
- Persist all state in `/workspace` (memory.md, tasks/, site/, assets/, tenant.sqlite) or via external services.
...
- For Stripe Checkout links, do NOT pass the URL through send_message. Instead, ask the
  interaction-agent to call mcp__claudius-chat__send_payment_link with:
...
- When you receive an EVENT indicating domain payment, purchase via:
  printf "y\n" | vercel domains buy <domain> [--token $VERCEL_TOKEN] [--scope $VERCEL_SCOPE]
- Update the billing record via mcp__claudius-chat__record_billing_status
